CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_trips`(in MIN_date date, in MAX_date date)
BEGIN
DECLARE counter INT DEFAULT 1;
DECLARE incrementer INT DEFAULT 1;
        ...
        if ( flight_id_departure is not null ) then
			...
			if ( flight_id_return is not null ) then 
                ...
                while (incrementer <=3) do
					select id into hotel_id from hotel where city_id = FLOOR((RAND() * 21)+1) limit 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_airport`()
BEGIN
DECLARE counter INT DEFAULT 1;
...
BEGIN
	...
	insert into city(`id`, `name`, `country_id`) values (6, 'St. Johns', 2);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_continent`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_flights`(in MIN_date date, in MAX_date date)
BEGIN
DECLARE counter INT DEFAULT 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_hotel`()
BEGIN
